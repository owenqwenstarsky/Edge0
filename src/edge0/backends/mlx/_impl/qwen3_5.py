...
#
# edge0 vendored copy of mlx-lm 0.31.0 ``mlx_lm/models/qwen3_5.py``,
# byte-identical EXCEPT: (1) the import block rewritten to absolute
# paths — infra bits (base/cache/gated_delta) point at the pinned
...
from edge0.backends.mlx._impl.qwen3_next import Qwen3NextRMSNormGated as RMSNormGated
from edge0.backends.mlx._impl.qwen3_next import Qwen3NextSparseMoeBlock as SparseMoeBlock
    ...
    def __call__(
        ...
        for li, (layer, c) in enumerate(zip(self.layers, cache)):
            ...
            if async_eval_per_layer:
                # Fire this layer's graph on the GPU while the CPU keeps
                # building the next layer (overlaps the whole-layer weight
                # load of the E3b prefill path).  The final logits eval
